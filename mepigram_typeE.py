...
python mepigram_typeE.py POSset.faa NEGset.faa backgroundmodel graph output.meme maxNo.motif
'''
def sortlex(L):
    L=map(str,L)
...
basetonum = lambda x: ''.join([{'A':'0','C':'1','G':'2','T':'3','E':'4'}[B] for B in x])
def revcompl(word):
    revComp=''
...
def revcompl_num(word):
    revComp=''
    transdict={'0':'3','1':'2','2':'1','3':'0'}
    index=0
    while index <len(word):
        if word[index] == '4' :#if is methylated C
            if index <len(word)-1 and word[index+1]=='2': #if G is next to it
                revComp+='24'
                index+=2
            else:
                revComp+='2'
...
#ref2Countmatrix is a dict of dict, I think
    ...
    for offset in offsets:
        ...
        for i in range(offset,WORDLENGTH+offset):
            #print "i is", i
            for j in range(0,5): #use 4 because of 4 bases
                #print "j is",j
        ...
        for i in range(offset, WORDLENGTH + offset):
            ...
            if colTotal > 0:
                colTotal += pseudocount*5 #multiply by the number of base types
...
if len(argv)<=1: #no arguments:
	print "Usage: python mepigram_typeE.py foreground dinucleotide-shuffled-foreground background graphdir resultMEMEfile maxmotiftofind"
	sys.exit(1)
...
if "background_typeE-" in WHOLEGENOME8MERCOUNTS and ".tsv" in WHOLEGENOME8MERCOUNTS:
    #WHOLEGENOME8MERCOUNTS =~ /background-(\d+)\.tsv/ ,is the variable matches the form specified?
    temp = WHOLEGENOME8MERCOUNTS.split("background_typeE-") 
...
elif "background_met-" in WHOLEGENOME8MERCOUNTS and ".tsv" in WHOLEGENOME8MERCOUNTS:
    #WHOLEGENOME8MERCOUNTS =~ /background-(\d+)\.tsv/ ,is the variable matches the form specified?
    temp = WHOLEGENOME8MERCOUNTS.split("background_met-") 
    WORDLENGTH = int(temp[1].split('.')[0])  #word length is from the name of the WHOLEGENOME8MERCOUNTS
    print "Word length is "+ str(WORDLENGTH) 
else:
    print "The format of the background file name is incorrect. Can't extract the 'word length' from it using regular expression\n\nFile name format should be \"background_typeE-WORDLENGTH.tsv\" where WORDLENGTH = the length of the word used. Instead I found %s.\n" % WHOLEGENOME8MERCOUNTS
    sys.exit()
        ...
        if wordsForTheFirstPart > STARTMOTIFWORDLIMIT:
            ...
            if intialMotifScore>0:
                ...
                for i in range(WORDLENGTH +2*PADDINGROUNDS):
                    colTotal = 0
                    #print "i is",i
                    for j in range(5): #use 5 instead of 4
...
for motifID in range(len(motifScores)): 
    ...
    #print motifname
    head="letter-probability matrix: alength= "+str(5)+" w= "+str(len(motifs[motifID]))+" nsites= "+str(motifsWordsUsed[motifID])+" score= "+str(motifScores[motifID])
    #print head
